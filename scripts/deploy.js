const main = async () => {
    ...
    let txn = await domainContract.register("origin",  {value: hre.ethers.utils.parseEther('0.1')});
    await txn.wait();
    console.log("Minted domain origin.dgtest");
  
    txn = await domainContract.setRecord("origin", "I am the Original Goat");
    await txn.wait();
    console.log("Set record for origin.dgtest");
